                ...
                children: [
                  Row(
                    children: [
                      ...
                      // Keeps clear air between the wordmark and the NEON
                      // pill even when the wordmark is at its widest.
...
///
/// On the launch screen rather than buried in an about page, because the
/// misunderstanding it heads off -- "this is a maps app, I'll navigate with
/// it" -- happens on first open or never. Someone following RoadScan instead
/// of Google Maps is exactly the failure mode worth spending screen space on.
///
/// The storage line is a factual claim about behaviour, so it has to stay true
/// if the app ever starts persisting positions: see LocationService, which
/// keeps the last fix in memory only, and SupabaseService, which uploads a
/// coordinate only as part of a hazard report the user explicitly submits.
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(Icons.shield_outlined, size: 17, color: c.accent),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 3),
                Text(
                  'RoadScan runs alongside it, warning you about severe road '
                  'conditions ahead through alerts and notifications. It does '
                  'not give directions. Your location is never stored.',
                  style: TextStyle(
                    color: c.textSecondary,
                    fontSize: 11.5,
                    height: 1.35,
...
class _Wordmark extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Two bakes of the same artwork, picked by theme.
    //
    // The supplied wordmark is white "ROAD" + green "SCAN" on transparency,
    // which is drawn for a dark ground -- on the light theme the white half
    // vanishes into the background and the name reads as "SCAN". The light
    // variant recolours only the neutral pixels to dark ink; both are
    // generated by tool/make_icons.py from roadscan-nobg.png.
    final light = context.rs.kind == AppThemeKind.light;
    return Align(
      alignment: Alignment.centerLeft,
      // Bounded on BOTH axes, not just height.
      //
      // Height alone lets the image take its intrinsic width, and at this
      // artwork's 7.8:1 ratio that ran the end of "SCAN" underneath the NEON
      // pill on a 480dp-wide phone. With contain and two bounds it scales to
      // whichever limit bites first and can never reach the controls.
      child: ConstrainedBox(
        constraints: const BoxConstraints(maxHeight: 24, maxWidth: 168),
        child: Image.asset(
          light
